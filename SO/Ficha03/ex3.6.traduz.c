...
int translate(const char * language, const char * translator) {
	...
	printf("Please enter a word to translate to %s:", language);
	fflush(stdout);
	scanf("%24s",word);
	...
	switch (pid) {
		...
		default:
			if(wait(&status) == -1) { //we need to wait for the tranlation process to end
...
int main(int argc, char** argv) {
	...
	do {
		printf("Select an option (x-exit; i-English dictionary; f-French dictionary):");
		scanf("%c",&option);
		switch (option) {
			case 'x':
